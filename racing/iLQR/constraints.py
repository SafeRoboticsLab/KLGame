...
class Constraints:
  ...
  def _road_boundary_cost(
    ...
    c_r = self.q1_road * np.exp(np.clip(self.q2_road * b_r, _min_val, _max_val))
    # Left Bound.
    b_l = -dis - (self.track_width_L - self.r)

    c_l = self.q1_road * np.exp(np.clip(self.q2_road * b_l, _min_val, _max_val))
...
class Constraints_racing(Constraints):

  def __init__(self, config):
    """
    Specialized Constraints class that accounts for race tracks with variable width.
    """

    super(Constraints_racing, self).__init__(config)
  ...
  def _road_boundary_cost(
    ...
    Args:
        states (np.ndarray): 4xN array of state.
        closest_pt (np.ndarray): 6xN array of each state's closest point [x,y] on the track.
    ...
    width_R = np.sqrt((closest_pt[0, :] - closest_pt[2, :])**2
                      + (closest_pt[1, :] - closest_pt[3, :])**2)
    b_r = dis - (width_R - self.r)
    # b_r = dis - (width_R-0.)
    c_r = self.q1_road * np.exp(np.clip(self.q2_road * b_r, _min_val, _max_val))

    # Left Bound.
    width_L = np.sqrt((closest_pt[0, :] - closest_pt[4, :])**2
                      + (closest_pt[1, :] - closest_pt[5, :])**2)
