...
class Dynamics:
  ...
  def __init__(self, idx: int, config: Struct, ref_vel_scaling: float = 1.0):
    ...
    self.ref_vel_scaling = ref_vel_scaling  # scaling factor of the reference velocity
  ...
  def dct_time_dyn(self, state: ArrayImpl, control: ArrayImpl) -> ArrayImpl:
    ...
    _dt = self.dt
    _wb = self.wheelbase
    d_x = ((state[2] * _dt + 0.5 * control[0] * _dt**2) * jnp.cos(state[3]))
    d_y = ((state[2] * _dt + 0.5 * control[0] * _dt**2) * jnp.sin(state[3]))
    d_v = control[0] * _dt
    d_psi = ((state[2] * _dt + 0.5 * control[0] * _dt**2) * jnp.tan(control[1]) / _wb)
