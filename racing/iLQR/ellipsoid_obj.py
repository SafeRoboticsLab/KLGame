...
class EllipsoidObj():
  ...
  def __init__(
      self, q: np.ndarray = np.array([]), Q: np.ndarray = np.array([[], []]), r=None, n_circ=None,
      center_L=None, major_axis=None
  ):
  ...
  def rho(self, L: np.ndarray, eps: float = 1e-10) -> Tuple[np.ndarray, np.ndarray]:
    """
  ...
  def __matmul__(self, A: np.ndarray) -> EllipsoidObj:
    ...
    return EllipsoidObj(
        q=q, Q=Q, r=self.r, n_circ=self.n_circ, center_L=self.center_L, major_axis=new_major_axis
    )
  ...
  def __add__(self, b: np.ndarray) -> EllipsoidObj:
    ...
    return EllipsoidObj(
        q=self.q + b, Q=self.Q, r=self.r, n_circ=self.n_circ, center_L=self.center_L,
  ...
  def is_internal(self, x: np.ndarray, abs_tol: float = 1e-7, slack: float = 0.) -> bool:
    """
  ...
  def plot(
      self, ax: matplotlib.axes.Axes, color: str = 'r', dims: List = None, N: int = 200,
    ...
    if E.dim() == 2:
      ...
      L = np.concatenate((np.cos(phi).reshape(1, N), np.sin(phi).reshape(1, N)), axis=0)
      _, x = E.rho(L)
  ...
  def obstacle_derivative(
      self, state: np.ndarray, center_L: float, obs: EllipsoidObj, q1: float, q2: float
    ...
    for i in range(self.n_circ):
      ...
      c_x_temp[3, :] = -(
          center_L[i] * q2 * c * (y*ct - obs.center[1, :] * ct - x*st + obs.center[0, :] * st)
      ) / d
